...
const Settings = () => {
  ...
  useEffect(() => {
    const fetchAdminDetails = async () => {
      try {
        ...
        setError(err.response?.data?.msg || 'Failed to load admin details');
      }
    };
    fetchAdminDetails();
  }, []);
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      const updateData = {};
      if (name && name !== adminDetails.name) updateData.name = name;
      if (email && email !== adminDetails.email) updateData.email = email;
      ...
      const response = await api.put('/admin/update-profile', updateData);
